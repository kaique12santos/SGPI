...
/* ===============================
   Funções de Semestre (SIMPLIFICADA)
=============================== */

// Busca o semestre explicitamente ATIVO no banco
async function getSemestreAtivo(conn) {
  const [rows] = await conn.query(`SELECT id FROM Semestres WHERE ativo = 1 LIMIT 1`);
  return rows.length ? rows[0].id : null;
}

// Decide qual semestre usar: O solicitado ou o Ativo
async function resolverSemestre(conn, semestreSolicitado) {
  if (semestreSolicitado && !isNaN(Number(semestreSolicitado))) {
    return Number(semestreSolicitado);
  }
  // Se não pediu histórico, retorna o ativo (padrão)
  return await getSemestreAtivo(conn);
}
...
router.get('/coordenador/total-projetos', async (req, res) => {
  ...
  try {
    conn = await getConnection();
    const semestreId = await resolverSemestre(conn, req.query.semestre_id);
...
router.get('/professor/reconsideracoes/:id', async (req, res) => {
  ...
  try {
    conn = await getConnection();
    // Reconsiderações pendentes independem do semestre, o professor precisa ver tudo
    const [rows] = await conn.query(
      `SELECT
        COUNT(rc.id) AS total,
        SUM(CASE WHEN rc.data_resposta IS NOT NULL THEN 1 ELSE 0 END) AS respondidos
...
router.get('/professor/semestres-anteriores/:id', async (req, res) => {
  ...
  try {
    conn = await getConnection();
    // Lista semestres onde o professor atuou
    const [rows] = await conn.query(
      `SELECT DISTINCT s.id, s.ano, s.periodo, 
              CONCAT(s.periodo, 'º Semestre de ', s.ano) AS descricao
       FROM Disciplinas_Ofertas dof
