...
async function carregarAtividades() {
  try {
    const atividades = await listarAtividades(usuarioId, baseEndpoint); // já JSON
...
function criarCardAtividade({ id, titulo, descricao, semestre, prazo_entrega, criterios_avaliacao }) {
  ...
  div.appendChild(criarBotao("Ver", "btn-Ver", () =>
    mostrarDetalhes(div, { titulo, descricao, semestre, prazo_entrega, criterios_avaliacao })
  ));
  div.appendChild(criarBotao("Alterar", "btn-alterar", () =>
    abrirModalEdicao(div, { id, titulo, descricao, semestre, prazo_entrega, criterios_avaliacao })
  ));
...
function abrirModalEdicao(div, atividade) {
  ...
  document.getElementById("editar-atividade").addEventListener("submit", async (e) => {
    ...
    try {
      const data = await atualizarAtividade(id, dados, baseEndpoint); // já JSON
      if (!data.success) {
...
async function excluirAtividade(div, atividadeId) {
  ...
  try {
    const data = await excluirAtividadeService(atividadeId, usuarioId, baseEndpoint); // já JSON
