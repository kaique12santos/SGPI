...
async function obterDadosUsuario() {
    try {
        ...
        const response = await fetch(`/perfil/usuario/${userId}`, {
            method: 'GET',
            headers: {
...
async function atualizarPerfil(event) {
    ...
    try {
        const response = await fetch(`/perfil/atualizar-perfil/${userId}`, {
            method: 'PUT',
            body: formData
        });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (botaoSalvar) {
        botaoSalvar.addEventListener('click', atualizarPerfil);
    }
});
